	...
	for ( int ui=0; ui<userID.length; ++ui )
	{
        ...
        if (tracking)
        {
          ...
          if(!newInput.equals(lastNote))
          {
            relevatedCount = 0;
            lastNote = newInput;
            println("count set to 0..(are u on moving)");
            isMovingMan = true;
          }
          
          else if( relevatedCount == 10 && isMovingMan)
          {
            relevatedCount = 0;
            println("count set to 0.. added note "+newInput);
            notes.append(newInput);
            println("PIO PAN LE NOTE: "+notes);
            isMovingMan = false;
            playNotes(newInput);
          }
...
String detect_user_behaviour() 
{
	...
	if(confidenceLH > 0.5)
	{
		...
		{
                        println("LEFT HAND Z: "+leftHand.z);
                        println("RIGHT HAND Z: "+rightHand.z);
...
//		else
//		{
//			println("Notes list too small to make a comparison");
//		}
//	}
//}
